"""
Step 1: Extract tidal harmonics from the ROMS database (self-contained).

Streams tide_data_east.dbf, keeps points inside all 15 East Coast state
bounding boxes (+ buffer), filters to depth >= 10 m, and saves harmonic
ellipse parameters for 10 tidal constituents.

Self-contained copy for the velocity_distribution experiment: all parameters
are hardcoded here (no import from optimization/vp/config). Only the shared
input files (the DBF and the state-boundaries CSV) are referenced by path.

Input:  inputs/roms/tide_data_east.dbf
        optimization/vp/config/east_coast_state_boundaries.csv
Output: experiments/velocity_distribution/results/harmonics.nc
"""
...
def main():
    ...
    print("Region: all 15 East Coast state boxes (pooled)")
