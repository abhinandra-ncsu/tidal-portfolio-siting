...
Diagnosis fork (turbine_modification Stage 1):
  - MIN_DEPTH_M = 2.0 (device-agnostic; lower than canonical 10.0 m)
  - Pooled scope (no state filter)
  - Output isolated under experiments/turbine_modification/diagnosis/results/

Streams tide_data_east.dbf, keeps points inside East Coast state bounding
boxes (+ buffer), filters to depth >= MIN_DEPTH_M, and saves harmonic ellipse
parameters for 10 tidal constituents.

Input:  <repo>/inputs/roms/tide_data_east.dbf
        <repo>/optimization/vp/config/east_coast_state_boundaries.csv
Output: <this-folder>/../results/harmonics.nc
"""
...
DBF_PATH = os.path.join(REPO_ROOT, "inputs", "roms", "tide_data_east.dbf")
BOUNDARIES_CSV = os.path.join(
    REPO_ROOT, "optimization", "vp", "config", "east_coast_state_boundaries.csv"
)
RESULTS_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "results"))
OUTPUT_PATH = os.path.join(RESULTS_DIR, "harmonics.nc")

# --- Parameters (diagnosis fork: see header) ---
MIN_DEPTH_M = 2.0
BBOX_BUFFER_DEG = 0.15
STATES = None  # pooled — all east coast
...
def save_dataset(arrays, out_path):
    ...
    ds = xr.Dataset(
        ...
        attrs={
            ...
            "scope": "diagnosis (turbine_modification Stage 1)",
...
def main():
    ...
    print("Scope: pooled (all east coast)")
