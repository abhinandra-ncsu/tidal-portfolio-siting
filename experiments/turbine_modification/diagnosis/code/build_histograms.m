...
%   - Reads harmonics.nc from this folder's ../results/
%   - Writes histograms.nc to the same location
%   - Uses canonical T_TIDE at optimization/vp/t_tide/ (no copy)
%   - Config inlined (no dependency on optimization/vp/config/config.m)
%
...
ttideDir = fullfile(repoRoot, 'optimization', 'vp', 't_tide');
...
%% Parameters (inlined from optimization/vp/config/config.m)
...
ncwriteatt(outputFile, '/', 'title', 'Tidal current speed histograms — US East Coast (diagnosis fork)');
...
ncwriteatt(outputFile, '/', 'scope', 'diagnosis (turbine_modification Stage 1)');
