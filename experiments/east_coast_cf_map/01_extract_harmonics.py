"""
Step 1: Extract tidal harmonics from the ROMS database (standalone).

Streams tide_data_east.dbf, keeps points inside East Coast state bounding
boxes (+ 0.15° buffer), filters to depth >= 10 m, and saves harmonic
ellipse parameters for 10 tidal constituents.

This is a self-contained copy of optimization/vp/01_extract_harmonics.py
at pooled scope — the experiment owns its own extraction so it can be
re-run from scratch without touching the VP pipeline.
...
OUTPUT_PATH = os.path.join(SCRIPT_DIR, "harmonics.nc")
...
def load_bounding_boxes(csv_path, buffer_deg):
    """Load East Coast state bboxes, buffered. Returns list of (S, N, W, E)."""
...
def main():
    ...
    print(f"Bbox buffer: {BBOX_BUFFER_DEG} deg")
