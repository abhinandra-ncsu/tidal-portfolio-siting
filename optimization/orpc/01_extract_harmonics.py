...
boxes (+ buffer), filters to MIN_DEPTH_M <= depth <= MAX_DEPTH_M (ORPC
operating window 18-40 m), and saves harmonic ellipse parameters for 10
tidal constituents.

Input:  inputs/roms/tide_data_east.dbf
        config/east_coast_state_boundaries.csv
Output: results/orpc/<group>/harmonics.nc
"""
...
from config.config import (
    MIN_DEPTH_M, MAX_DEPTH_M, BBOX_BUFFER_DEG, STATES, GROUP, get_results_dir,
)
...
def extract_points(dbf_path, boxes, min_depth, max_depth):
    """Stream DBF, keep East Coast points with min_depth <= depth <= max_depth."""
    ...
    for rec in table:
        ...
        if n_scanned % 200_000 == 0:
            ...
            print(f"  {n_scanned:,} scanned, {len(lons):,} kept, "
                  f"{n_shallow:,} too shallow, {n_deep:,} too deep "
                  f"({elapsed:.0f}s)")
...
def save_dataset(arrays, out_path):
    ...
    ds = xr.Dataset(
        ...
        attrs={
            "title": f"Tidal harmonic constituents — US East Coast, {MIN_DEPTH_M} <= depth <= {MAX_DEPTH_M} m",
...
def main():
    ...
    print(f"Depth window: {MIN_DEPTH_M} <= depth <= {MAX_DEPTH_M} m")
    ...
    print("Streaming DBF...")
    arrays = extract_points(DBF_PATH, boxes, MIN_DEPTH_M, MAX_DEPTH_M)
