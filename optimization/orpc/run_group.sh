#!/usr/bin/env bash
# Run the full ORPC TidGen 2.0 pipeline for a named group of states.
#
# Usage:
#   ./run_group.sh <group_name> <state1,state2,...>
#
# Example — New England + NY (the planned run):
#   ./run_group.sh new_england_ny \
#       Maine_coastline,New_Hampshire_coastline,Massachusetts_coastline,Rhode_Island_coastline,Connecticut_coastline,New_York_coastline
#
# Writes outputs to results/orpc/<group_name>/.
...
RESULTS_DIR="$REPO_DIR/results/orpc/groups/$GROUP"
mkdir -p "$RESULTS_DIR"
...
echo "========================================================"
echo "ORPC TidGen 2.0 group run: $GROUP"
echo "States: $STATES"
echo "Started: $(date)"
